...
The Simulation Investment Desk distinguishes two paper-only simulation origins: `user_recorded` paper trades entered by Harness Engineering / the human user, and `system_generated_learning` simulations generated by HK Alpha Team to validate recommendation quality and produce reviewable learning proposals. Both origins remain advisory-only, auditable, and human-in-the-loop.
