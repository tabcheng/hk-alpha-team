...
| 008 | Simulation Desk MVP | In Progress | Task 008G adds governance-sensitive, contract-changing, schema-changing dual simulation-origin product foundation after Harness Engineering approved non-real-money productization gates. `user_recorded` and `system_generated_learning` records are now distinguished in docs, schema planning, API contract text, and local validation helpers/tests. Task 008 / M5 remains In Progress; no endpoint runtime, persistence writes, production Supabase, broker integration, live market data, secrets, deployment, autonomous order placement, or real-money trading automation is implemented or authorized. |
...
- Harness Engineering approved opening non-real-money productization gates after Task 008F for schema/contract alignment, local/test migration planning, persistence design, endpoint runtime planning, paper trading recordkeeping, system-generated learning simulations, audit events, and future non-real-money runtime sequencing.
- Task 008G adopts the dual simulation-origin model: `user_recorded` records are human/user paper-trade records, while `system_generated_learning` records are Simulation Investment Desk validation simulations that may create reviewable learning proposals. Real-money trading, autonomous real-money execution, broker execution APIs, production Supabase connection, secrets, silent auto-application of learning proposals, and hidden/overwritten losing simulations remain prohibited; Task 008 / M5 remains In Progress.
...
- Task 008 Simulation Desk MVP and M5 remain In Progress. Task 008D adds persistence gate readiness and migration alignment planning only after Task 008A planning, Task 008B local contract fixtures/validators, and Task 008C local paper-order validation service/stub; it does not implement SQL migration, endpoint runtime, persistence writes, Supabase client, production Supabase, paper-order creation, paper-portfolio runtime, strategy recommendation persistence, broker integration, or real-money trading automation.
- The locked endpoint names, success/error envelope, `analysis_status`, and `workflow_phase` remain unchanged; Task 008G clarifies the future `POST /api/v1/simulation/paper-orders` contract with `simulation_origin` while preserving the endpoint name and required response envelope.
- The Codex PR Factory governance workflow is recorded in `docs/20-codex-pr-factory.md`.
- Task 007 remains Completed and M4 remains Completed; Task 008 / M5 has progressed from Task 008A planning, Task 008B pure local contract validation, and Task 008C pure local paper-order intent validation to Task 008D persistence gate readiness planning with no SQL migration, IO, HTTP, runtime endpoints, persistence writes, Supabase client, production Supabase, broker, paper-order creation, production infrastructure, or real-money trading scope.
