...
| Local source | Local record shape | Canonical table from `docs/08` | Canonical `docs/08` fields already represented | Fixture-only / governance requirement / future schema decision needed | Required future persistence relationships / notes | Gate before implementation |
|---|---|---|---|---|---|---|
| Task 008B | `paper_portfolio` | `paper_portfolios` | `portfolio_uuid`, `name`, `base_currency`, `starting_cash`, `status`, `created_at` | None identified for this row. | Optional `owner_profile_id` remains unresolved for v1 ownership/RLS planning. The record remains paper-only. RLS/auditability readiness must be reviewed before any future runtime. | SQL migration gate, Supabase client gate, persistence write gate, paper-portfolio runtime gate. |
| Task 008B / 008C | `paper_order_intent` | `paper_orders` | `portfolio_id`, `stock_id`, `strategy_recommendation_id`, `side`, `order_type`, `quantity`, `limit_price`, `status`, `submitted_at`, `filled_at`, `created_at` | Task 008C local `symbol` is a validation input, not a `paper_orders` column. Future persistence must decide symbol-to-`stocks.id` resolution before writing. | Requires `portfolio_id -> paper_portfolios.id`, `stock_id -> stocks.id`, and `strategy_recommendation_id -> strategy_recommendations.id` alignment. Task 008C validates intent only and returns `would_create_order = false`. | SQL migration gate, Supabase client gate, persistence write gate, endpoint runtime gate, paper-order creation gate. |
| Task 008B | `paper_position` | `paper_positions` | `portfolio_id`, `stock_id`, `opened_from_order_id`, `side`, `quantity`, `avg_entry_price`, `avg_exit_price`, `status`, `opened_at`, `closed_at`, `created_at` | Task 008B local `outcome_label` and `losing_outcome_visible` are validation/governance invariants, not approved `paper_positions` columns. | Requires `portfolio_id -> paper_portfolios.id`, `stock_id -> stocks.id`, and `opened_from_order_id -> paper_orders.id`. Losing outcomes must remain visible and must not be overwritten. | SQL migration gate, persistence write gate with retention / append-only readiness review, paper-portfolio runtime gate. |
| Task 008B | `portfolio_snapshot` | `portfolio_snapshots` | `portfolio_id`, `snapshot_time`, `nav`, `cash`, `gross_exposure`, `net_exposure`, `drawdown_pct`, `created_at` | None identified for this row. | Requires `portfolio_id -> paper_portfolios.id`. Future uniqueness policy must preserve the canonical unique `(portfolio_id, snapshot_time)` expectation without hiding historical drawdowns. | SQL migration gate, persistence write gate with retention / append-only readiness review, paper-portfolio runtime gate. |
| Task 008B | `trade_review` | `trade_reviews` | `paper_position_id`, `outcome_label`, `review_notes`, `mistake_tags_json`, `what_worked_json`, `what_failed_json`, `reviewed_at`, `created_at` | Task 008B local `losing_outcome_visible` is a validation/governance invariant, not an approved `trade_reviews` column. | Requires `paper_position_id -> paper_positions.id`. Losing trades must remain visible in reports, and review notes must not rewrite historical recommendations or paper positions. | SQL migration gate, persistence write gate with retention / append-only readiness review, audit-event creation gate. |
| Task 008B | `learning_proposal` | `learning_proposals` | `source_type`, `source_id`, `title`, `proposal_text`, `expected_impact`, `risk_of_change`, `status`, `created_at`, `updated_at` | Task 008B local `reviewable` and `auto_applied` are validation/governance invariants, not approved `learning_proposals` columns. | Uses typed source metadata. Proposals must remain reviewable and must not silently alter production behavior. Future status transitions need review evidence before persistence. | SQL migration gate, persistence write gate, strategy recommendation persistence gate where coupled, audit-event creation gate. |
| Task 008B | `audit_event` | `audit_events` | `event_uuid`, `event_type`, `entity_type`, `entity_id`, `actor_type`, `actor_id`, `event_payload_json`, `created_at` | None identified for this row. | Uses polymorphic entity references via `entity_type` and `entity_id`. Future persisted events must be immutable and append-only. | SQL migration gate, persistence write gate with retention / append-only readiness review, audit-event creation gate. |

### Fixture-Only Governance Invariant Note

The Task 008B / 008C local validation fields `paper_position.outcome_label`, `paper_position.losing_outcome_visible`, `trade_review.losing_outcome_visible`, `learning_proposal.reviewable`, and `learning_proposal.auto_applied` are local validation/governance invariants only. They are not approved canonical database columns in `docs/08-supabase-schema-design.md`.

Future persistence work must decide whether each invariant is represented as a new canonical column, a JSON payload field, derived report logic, or a validation-only invariant that is never persisted directly. Adding any of these as database columns requires explicit Harness Engineering approval plus same-PR updates to `docs/08-supabase-schema-design.md`, SQL migration files, tests, `docs/decision-log.md`, `docs/progress-log.md`, and `docs/11-project-status.md`. Task 008D does not approve those schema changes.
